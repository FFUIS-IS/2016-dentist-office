...
namespace ZubarskaOrd.Forms
{
    public partial class PasswordChangingForm : Form
    {
        ...
        private void changingPassword()
        {
            ...
            SqlCeCommand cm = new SqlCeCommand("SELECT password FROM LoginFormTable WHERE AdministratorId = '" + LoginForm.userIdentity + "'", connection);
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (password == oldPassword)
                    {
                        ...
                        {
                            SqlCeCommand command = new SqlCeCommand("UPDATE LoginFormTable SET Password = '" + HashPassword.HashNewPassword(newPassword) + "' WHERE AdministratorId = '" + LoginForm.userIdentity + "'", connection);
